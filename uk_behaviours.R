...
questionnaire$gender[questionnaire$gender == 4 | questionnaire$gender ==  5] <- 3 #code trans as other
questionnaire$sexuality[questionnaire$gender == 2 & questionnaire$sexuality == 2] <- 3 #make sure all gay women are coded as lebsians
questionnaire$sexuality[questionnaire$sexuality == 6 | questionnaire$sexuality == 7] <- 5 #code asexual and prefer.not as other 
questionnaire <- questionnaire[questionnaire$HIV != 2, ]  #Remove HIV+
...
questionnaire$sex.in.3.years            <- factor(questionnaire$sex.in.3.years, labels = c('yes', 'no', 'do.not.know'))
...
glm.formula <- behaviour.broke ~ gender + sexuality +
                sexuality:activity.participate.anal + sexuality:activity.participate.vaginal +
...
lm.best  <- MASS::stepAIC(glm(behaviour.broke ~ gender + sexuality + condom  + 
                                ...
                                sexuality:activity.participate.anal, data = questionnaire, family = binomial(link = 'logit')), direction = "backward")
prob <- predict(lm.best,type=c("response"))
g <- roc(as.numeric(questionnaire$behaviour.broke) ~ prob)
auc(g)
logistic.regression.or.ci(lm.best)

lm.best.subset <- glm(lm.best$formula, family = binomial(link = 'logit'), data = questionnaire[1:3000, ])
prob        <- predict(lm.best.subset,type=c("response"), newdata = questionnaire[-c(1:3000), ])
prediction <- rbinom(length(prob), 1, prob)
table(prediction, questionnaire[-c(1:3000), ]$behaviour.broke)
...
#First Suggestion
glm.formula <- behaviour.forgot ~ gender + sexuality + condom  + 
  activity.participate.anal + activity.participate.vaginal+ 
  sexuality:activity.participate.anal + sexuality:activity.participate.vaginal
lm.full     <- glm(glm.formula, family = binomial(link = 'logit'), data = questionnaire)
prob        <- predict(lm.full,type=c("response"))
...
lm.best  <- MASS::stepAIC(glm(behaviour.forgot ~ gender + sexuality + condom  + 
                                activity.participate.anal + activity.participate.vaginal+ 
                                sexuality:activity.participate.anal + sexuality:activity.participate.vaginal, data = questionnaire, family = binomial(link = 'logit')), direction = "backward")
prob <- predict(lm.best,type=c("response"))
g <- roc(as.numeric(questionnaire$behaviour.forgot) ~ prob)
auc(g)
logistic.regression.or.ci(lm.best)
...
prob        <- predict(lm.best.subset,type=c("response"), newdata = questionnaire[-c(1:3000), ])
prediction <- rbinom(length(prob), 1, prob)
table(prediction, questionnaire[-c(1:3000), ]$behaviour.forgot)
...
glm.formula <- behaviour.not.available ~ gender + sexuality +
  ...
  gender:activity.receive.anal
lm.full     <- glm( behaviour.not.available ~ sexuality + condom + 
                      activity.receive.anal + activity.perform.vaginal + sexuality:activity.perform.vaginal, family = binomial(link = 'logit'), data = questionnaire)
prob        <- predict(lm.full,type=c("response"))
...
#Begin with over fitted model
lm.best  <- MASS::stepAIC(glm(behaviour.not.available ~ gender + sexuality + condom  + 
                                activity.participate.anal + activity.participate.vaginal
                                , data = questionnaire, family = binomial(link = 'logit')), direction = "backward")
prob <- predict(lm.best,type=c("response"))
g <- roc(as.numeric(questionnaire$behaviour.not.available) ~ prob)
auc(g)
logistic.regression.or.ci(lm.best)
...
prob        <- predict(lm.best.subset,type=c("response"), newdata = questionnaire[-c(1:3000), ])
prediction <- rbinom(length(prob), 1, prob)
table(prediction, questionnaire[-c(1:3000), ]$behaviour.not.available)
...
lm.best  <- MASS::stepAIC(glm(outcome.diagnosis ~ gender + sexuality + condom  + 
                                activity.participate.anal + activity.participate.vaginal+ 
                                sexuality:activity.participate.anal + sexuality:activity.participate.vaginal, data = questionnaire, family = binomial(link = 'logit')), direction = "both")
prob <- predict(lm.best,type=c("response"))
...
full <- glm(outcome.diagnosis ~ gender + sexuality + condom  + 
      ...
      gender:activity.receive.anal, data = questionnaire)
allfits <- dredge(full)
...
lm.best  <- MASS::stepAIC(glm(outcome.no.test~ gender + sexuality + condom  + 
                                activity.participate.anal + activity.participate.vaginal+ 
                                sexuality:activity.participate.anal + sexuality:activity.participate.vaginal + gender:activity.participate.vaginal, data = questionnaire, family = binomial(link = 'logit')), direction = "back")
prob <- predict(lm.best,type=c("response"))
...
full <- glm(outcome.no.test ~ gender + sexuality + condom  + 
              activity.participate.anal + activity.participate.vaginal+ 
              sexuality:activity.participate.anal + sexuality:activity.participate.vaginal, data = questionnaire)
allfits <- dredge(full)



lm.best.no.condom  <- MASS::stepAIC(glm(outcome.no.test ~ gender + sexuality + condom  + 
                                          activity.participate.anal + activity.participate.vaginal+ 
                                          sexuality:activity.participate.anal + sexuality:activity.participate.vaginal, data = questionnaire[questionnaire$condom == "never", ]), direction = "back")
